...
public class LengthLongestPath {
    ...
    // so, we can maintain a stack which states the current level's path length
    //
    // For example:
    //    dir
    ...
    static int lengthLongestPath(String input) {
        ...
        for(String token : tokens) {
            int level = countTabs(token);
            //adding "/" into token size
            int tokenSize = token.replaceAll("\t", "").length() + 1;
            //if we have more levels in stack than level, remove
            ...
            if(token.contains(".")) {
                //remove "/" from last file
                curSize--;
                tokenSize--;
                maxSize = Math.max(curSize, maxSize);
            }
            
            stack.push(tokenSize);
        }
